...
// with manual button for names" — a private, dashboard-only
// (app/dashboard/admin/openplayspecial) copy of tv-display-client.tsx,
// identical in every other respect, plus one addition: a manual
// "Announce" button right on the display itself (reusing the exact same
// announceAssignmentAction the staff rotation board's own Announce
// button already calls), so the owner can watch AND trigger the call
// from one screen without needing to also have the rotation board open.
// The PUBLIC /tv display stays untouched — this is a deliberate fork,
// not a shared "mode" flag, so a change here can never leak an
// interactive control onto the public kiosk.
//
...
export function OpenPlaySpecialDisplayClient({
  initialData,
  ...
  // Manual Announce, reachable directly from this private display — same
  // underlying action the staff rotation board's own Announce button
  // calls. Fire-and-forget: the next poll (a few seconds away at most)
  // naturally picks up the new announcementRequestedAt token and speaks
  // it, same as it would for a press on the rotation board itself — no
  // separate refetch needed here.
  const [isAnnouncing, startAnnouncing] = useTransition();
  function handleManualAnnounce(assignmentId: string) {
    startAnnouncing(async () => {
      const result = await announceAssignmentAction({ assignmentId });
      if (result.error) {
  ...
  // Manual "Time's up" — owner request (2026-08-09): "it will only be
  // used to call out the players and to call if the time is up." Same
  // shape as handleManualAnnounce above, reusing the exact action the
  // staff rotation board's own "Time's up" button calls. Only ever
  // rendered for "op" (a running game) — see courtTimesUpToken's own
  // comment for why "op-pending" has no meaningful "time's up" yet.
  const [isCallingTimesUp, startCallingTimesUp] = useTransition();
  function handleManualTimesUp(assignmentId: string) {
    startCallingTimesUp(async () => {
      const result = await announceTimesUpAction({ assignmentId });
      if (result.error) {
        toast.error(result.error);
        return;
      }
      toast.success("Time's up called.");
    });
  }
      ...
      <div className={styles.courts}>
        {data.courts.map((court) => (
          <CourtCard
            ...
            onAnnounce={handleManualAnnounce}
            announcing={isAnnouncing}
            onTimesUp={handleManualTimesUp}
            callingTimesUp={isCallingTimesUp}
          />
        ))}
      </div>
...
function CourtCard({
  ...
  // Manual Announce (owner request, 2026-08-09) — only meaningful for
  // op/op-pending, which are the only states carrying an assignmentId.
  onAnnounce: (assignmentId: string) => void;
  announcing: boolean;
  // Manual "Time's up" (owner request, 2026-08-09) — only meaningful for
  // "op" (a running game); see courtTimesUpToken's own comment for why
  // "op-pending" never gets this button.
  onTimesUp: (assignmentId: string) => void;
  callingTimesUp: boolean;
}) {
  if (court.state === "free") {
    return (
      ...
      <div className={cls(styles.court, styles.opPending, nudging && styles.opPendingNudge)}>
        ...
        {court.assignmentId ? (
          <button
            type="button"
            disabled={announcing}
            onClick={() => onAnnounce(court.assignmentId!)}
            className="border-line text-bone hover:bg-navy-700 mt-2 self-start rounded-full border bg-transparent px-3 py-1 font-mono text-[11px] tracking-widest uppercase disabled:opacity-50"
          >
      ...
      {court.assignmentId ? (
        <div className="mt-2 flex gap-2">
          <button
            ...
            disabled={announcing}
            onClick={() => onAnnounce(court.assignmentId!)}
            className="border-line text-bone hover:bg-navy-700 self-start rounded-full border bg-transparent px-3 py-1 font-mono text-[11px] tracking-widest uppercase disabled:opacity-50"
          >
            {announcing ? "Announcing…" : "Announce"}
          </button>
          <button
            type="button"
            disabled={callingTimesUp}
            onClick={() => onTimesUp(court.assignmentId!)}
            className="border-line text-bone hover:bg-navy-700 self-start rounded-full border bg-transparent px-3 py-1 font-mono text-[11px] tracking-widest uppercase disabled:opacity-50"
          >
            {callingTimesUp ? "Calling…" : "Time's up"}
          </button>
        </div>
      ) : null}
    </div>
