...
import { resolveLockerMaintenanceAction } from "@/actions/locker.actions";
...
type Logs = Awaited<ReturnType<typeof lockerService.listMaintenanceLogs>>;

interface LockerMaintenanceLogListProps {
  lockerId: string;
  logs: Logs;
}

export function LockerMaintenanceLogList({ lockerId, logs }: LockerMaintenanceLogListProps) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();

  function handleResolve(logId: string) {
    startTransition(async () => {
      const result = await resolveLockerMaintenanceAction(logId, lockerId, {});
      if (result.error) {
