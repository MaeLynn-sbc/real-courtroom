...
export async function createProductAction(input: CreateProductInput): Promise<CreateProductActionState> {
...
export async function reorderProductsAction(input: ReorderProductsInput): Promise<ProductActionState> {
  const authz = await requireSystemAdmin("You don't have permission to manage products.");
  if (!authz.ok) {
    return { error: authz.error };
  }
  ...
  try {
    ...
    return { error: toActionError(error, { action: "reorderProductsAction", userId: authz.userId }) };
  }
}
