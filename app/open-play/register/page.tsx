...
export default async function OpenPlayRegisterPage({ searchParams }: OpenPlayRegisterPageProps) {
  ...
  const dayEnabledNights = upcomingNights.filter(
    (night) =>
      capacityDefaults.find((row) => row.dayOfWeek === night.dayOfWeek)
  ...
  const eligibleNights = dayEnabledNights
    ...
    .map((night) => ({
      date: toLocalDateValue(night.date),
      label: labelFormatter.format(night.date),
      // Same computeRemainingSeats the home page's own cards use, on the
      // same registeredCount getUpcomingNights already returns — never a
      // second, independent count that could disagree with the card
      // someone clicked through from.
      remainingSeats: computeRemainingSeats(night.capacity, night.registeredCount),
    }));
