...
import { OpenPlaySpecialDisplayClient } from "@/features/display/components/open-play-special-display-client";
...
// visible to me in the dashboard... with manual button for names."
// Same displayService.getDisplayData() call and live poll/speech-queue/
// token-diff behavior as app/tv/page.tsx, plus a manual per-court
// Announce button (see OpenPlaySpecialDisplayClient's own comment) —
// the one addition that makes this genuinely different from just
// visiting /tv while signed in. Lives under /dashboard/admin, so
// middleware.ts's /dashboard/:path* auth gate applies (unlike /tv,
// which is deliberately public/unguessable-free) and lib/rbac.ts's
