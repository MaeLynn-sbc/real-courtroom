...
// Mirrors app/api/expense-receipt/[key]/route.ts exactly — staff-only,
// same reason: a receipt must never be reachable at a guessable-but-
// unauthenticated static path.
export async function GET(_request: Request, { params }: { params: Promise<{ key: string }> }) {
  const authz = await requirePermission(
    PERMISSIONS.TOURNAMENTS_MANAGE,
    "You don't have permission to view tournament registration receipts.",
  );
  if (!authz.ok) {
