...
// Booking's twin of /api/booking-payment-proof/[key] — same staff-only,
// permission-gated shape, so a settle-time receipt/GCash confirmation
// photo is never reachable at a guessable-but-unauthenticated static
// path the way /public/uploads files are.
export async function GET(_request: Request, { params }: { params: Promise<{ key: string }> }) {
  const authz = await requirePermission(
    PERMISSIONS.BOOKINGS_MANAGE,
    "You don't have permission to view booking receipts.",
  );
  if (!authz.ok) {
