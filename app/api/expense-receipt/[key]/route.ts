...
// Mirrors app/api/booking-payment-proof/[key]/route.ts and
// app/api/open-play-registration-payment-proof/[key]/route.ts exactly —
// staff-only, same reason: a receipt must never be reachable at a
...
export async function GET(_request: Request, { params }: { params: Promise<{ key: string }> }) {
  const authz = await requirePermission(
    ...
    "You don't have permission to view expense receipts.",
  );
  if (!authz.ok) {
