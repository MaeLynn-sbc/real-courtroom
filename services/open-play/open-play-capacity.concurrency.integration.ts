/**
 * Hardening phase, fix 3/6 (BUILD-SPEC.md §0 process rule): closeSession's
 * "no open tabs" check and its status write were not atomic — a tab could
 * become non-zero in the gap between them, letting a session close with
 * real money still outstanding. Phase 7 built this guard specifically to
 * stop exactly that (BUILD-SPEC.md §9 correctness #6), and the guard was
 * itself check-then-act — the clearest example of why this audit was
