 ...
 *  - The line (Next up, After that, Then, Racks 1-12): every group moves
 *    up into an empty spot, whatever its size, keeping its order.
 *  - A group completed by a player who just played goes to the back.
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // 3, and a full group put on Rack 4 (leaving Racks 2-3 empty).
    ...
    assert((await slotOf(rack4.id)) === "RACK_5", "a group put on Rack 4 closes the gap to Rack 2");
    assert((await slotOf(rack1.id)) === "RACK_4", "Rack 1 keeps its place");
    console.log("PASS: groups close gaps in the line, keeping their order.");

    // 0. A one-player group moves up too, but can't go on court, and says why.
    const solo = await stage("RACK_8", 1);
    assert((await slotOf(solo.id)) === "RACK_6", "a one-player group also closes the gap");
    let refused = "";
    try {
    ...
    // 1. Next up goes on court. Every group behind moves up one — the
    //    3-player Rack 1 included (owner, 2026-09-17: "disregard the rule
    //    that 3 players will not proceed — for all boxes").
    await openPlayRotationService.assignPendingGroupToCourt(
      TEST_DATE,
      court.id,
      nextUp.id,
    ...
    assert((await slotOf(rack1.id)) === "THEN", "the 3-player Rack 1 moved up into Then");
    assert((await slotOf(rack4.id)) === "RACK_4", "the next rack moved up into Rack 1");
    assert((await slotOf(solo.id)) === "RACK_5", "the one-player group moved up into Rack 2");
    console.log("PASS: when Next up goes on court, every group moves up one, whatever its size.");

    // 2. Completing a group changes nothing about its place.
    await openPlayRotationService.addPlayerToStagedGroup(rack1.id, take(1)[0], owner.id);
    assert((await slotOf(rack1.id)) === "THEN", "a group completed by a fresh player stays put");
    ...
    // A group starts forming while that game is still on court, and
    // another forms behind it.
    const waitingGroup = await stage("RACK_6", 3);
    const behind = await stage("RACK_7", 2);
    ...
    const behindNow = await slotOf(behind.id);
    const lastSlot = (await prisma.stagedGroup.findMany({ where: { date: TEST_DATE } }))
      .map((g) => g.slot)
      .sort((x, y) => STAGED_SLOTS.indexOf(y) - STAGED_SLOTS.indexOf(x))[0];
    assert(
      behindNow === "RACK_6" && movedTo === "RACK_7" && lastSlot === "RACK_7",
      `a group completed by a just-played player moves behind everyone (behind group ${behindNow}, it ${movedTo})`,
    );
    const freshGroup = await stage("RACK_8", 3);
    await openPlayRotationService.addPlayerToStagedGroup(freshGroup.id, take(1)[0], owner.id);
    assert(
      (await slotOf(freshGroup.id)) === "RACK_8",
    ...
    assert(
      farSlot === "RACK_9",
      `a group on Rack 12 moves up to the first free rack, got ${farSlot}`,
    );
    ...
    const edited = await prisma.openPlayNightRegistration.findUniqueOrThrow({
      where: { id: regIds[1] },
    });
    ...
    const editedEntry = await prisma.queueEntry.findUniqueOrThrow({
      where: { registrationId: regIds[1] },
    });
    ...
    console.log(
      "PASS: editing a waiting player's skill and phone updates the queue and their profile.",
    );
    assert(
    ...
    // The returning player's group went to the back, then Rack 5 filled
    // behind it — it now sits in Rack 4.
    assert(
      listed.placement === "Rack 4",
      `the tab says where the player is, got "${listed.placement}"`,
    );
    console.log(
