...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert(
      (await prisma.player.count()) === playersBefore,
      "practice names never create a Player row",
    );
    ...
    assert(
      regs.every((r) => waitingIds.includes(r.id)),
      "every practice name is waiting in the practice line",
    );
    assert(
      (await prisma.playerTab.count({
        where: { registrationId: { in: regs.map((r) => r.id) } },
      })) === 0,
      "no tab is opened",
    );
    console.log(
      "PASS: practice players go straight into the practice line, with no Player row and no tab.",
    );

    // 2. Stage them, put them on a court, start and finish the game.
    const staged = await openPlayRotationService.stageManualGroup(
      date,
      "NEXT_UP",
      regs.map((r) => r.id),
      owner.id,
    ...
    assert(
      ...
      "a finished practice game opens no tab",
    );
    assert(
      (await prisma.tabLineItem.count({ where: { gameAssignmentId: assignment.id } })) === 0,
      "a finished practice game is never billed",
    );
    assert(
      (await prisma.sale.count({
        where: { openPlayNightRegistrationId: { in: regs.map((r) => r.id) } },
      })) === 0,
      "no sale is linked to a practice player",
    );
    assert((await prisma.sale.count()) === salesBefore, "no sale was created at all");
    console.log(
      "PASS: a full practice game (staged, on court, started, finished) bills nothing and records no sale.",
    );
    ...
    assert(
      bills.every(
        (b) =>
          b.items.length === 1 &&
          b.items[0].description.startsWith(`OP · ${court.name} · `) &&
          b.totalCents > 0,
      ),
      `each bill lists the game with court and time, got ${JSON.stringify(bills[0])}`,
    );
    assert(bills.every((b) => b.placement === "Waiting"), `after their game each player is back to Waiting, got ${bills.map((b) => b.placement).join(",")}`);
    ...
    assert(
      practiceBoard.courts.every((c) => !c.booked),
      "real bookings never mark a practice court booked",
    );
    console.log(
      "PASS: practice bills itemise finished games; practice courts are never blocked by real bookings.",
    );
    ...
    try {
      await openPlayRegistrationService.registerWeeknightWalkIn(
        date,
        { playerName: "Real Walkin", phone: "1", skillLevel: "NOVICE" },
        owner.id,
      );
    } catch {
      walkInRefused = true;
    ...
    assert(
      practiceFrame.practice === true && liveFrame.practice === false,
      "the frame says whether it is practice",
    );
    const practiceNames = [
      ...practiceFrame.courts.flatMap((c) => c.players.map((p) => p.name)),
      ...practiceFrame.queue,
    ];
    const liveNames = [
      ...liveFrame.courts.flatMap((c) => c.players.map((p) => p.name)),
      ...liveFrame.queue,
    ];
    assert(
      practiceNames.some((n) => n.startsWith("Practice")),
      "the practice frame shows practice names",
    );
    assert(
      !liveNames.some((n) => n.startsWith("Practice")),
      "the live frame never shows practice names",
    );
    ...
    console.log(
      "PASS: /rtv data is practice only while the switch is on; live data never includes practice.",
    );
    ...
    assert(
      first.added === 40 && again.added === 0,
      `sample players: first ${first.added}, again ${again.added}`,
    );
    const byLevel = await prisma.openPlayNightRegistration.groupBy({
      by: ["skillLevel"],
      where: { date },
      _count: true,
    });
    assert(
      byLevel.length === 4 && byLevel.every((g) => g._count === 10),
      `10 per level, got ${JSON.stringify(byLevel)}`,
    );
    ...
    console.log(
      "PASS: Add sample players adds 10 per skill level, once, with no players or sales.",
    );

    // 6. Clear practice.
    const cleared = await practiceService.clearPractice(owner.id);
    assert(
      cleared.players === 40 && cleared.games === 0,
      `clear reports what it removed, got ${JSON.stringify(cleared)}`,
    );
    assert(
      (await prisma.openPlayNightRegistration.count({ where: { date } })) === 0,
      "no practice names remain",
    );
    assert(
      (await prisma.gameAssignment.count({ where: { date } })) === 0,
      "no practice games remain",
    );
    assert(
      (await prisma.stagedGroup.count({ where: { date } })) === 0,
      "no practice staged sets remain",
    );
    assert(
      (await prisma.queueEntry.count({ where: { date } })) === 0,
      "the practice line is empty",
    );
