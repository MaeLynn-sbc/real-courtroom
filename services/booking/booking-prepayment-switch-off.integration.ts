 ...
 * OFF (the default — no Setting row at all, same as production today),
 * the public booking path is byte-for-byte unchanged. Calls
 * createPublicBooking (services/booking/public-booking.service.ts) — the
 * exact function actions/public-booking.actions.ts calls, minus only the
 * revalidatePath calls that can't run outside a real request context.
 *
...
async function main(): Promise<void> {
  ...
  // Confirm the switch reads as off right now. Deliberately NOT asserting
  // "no Setting row exists at all" — a row with value:false (e.g. left
  // behind by booking-prepayment-switch-on.integration.ts's own restore
  // step) is behaviorally identical to no row at all (getBooleanFlags'
  // "absence of a row means false" doctrine), and this test only cares
  // about the actual behavior, not which of those two equivalent states
  // produced it.
  ...
  await cleanUp(court.id);
  console.log(
    "PASS: with the prepayment switch off (the untouched default), createPublicBooking behaves exactly as it did before Phase 8 — instant CONFIRMED booking, Sale created immediately, no hold, no proof row.",
  );
  process.exit(0);
}
